...
#define UNKNOW_FILETYPE_ICON "ico_unknow.bmp"
...
static PT_PixelDatas GetFilePixelData(const char * filename)
{
    ...
    for(i = 0;i < ARRAYSIZE(g_TfileIconDesc);i++)
    {
        ...
        {
            return &g_TfileIconDesc[i].tPixeldatas;
        }
    }
    return &g_tFileIconPixelDatas;
}
    ...
    for (i = 0; i < g_iDirFileNumPerCol; i++)
    {
        for (j = 0; j < g_iDirFileNumPerRow; j++)
        {
            if (iDirContentIndex < iDirContentsNumber)
            {
                ...
                {
                    filePixelData = GetFilePixelData(aptDirContents[iDirContentIndex]->strName);
                    //PicMerge(atLayout[k].iTopLeftX, atLayout[k].iTopLeftY, &g_tFileIconPixelDatas, &ptVideoMem->tPixelDatas);
                    PicMerge(atLayout[k].iTopLeftX, atLayout[k].iTopLeftY, filePixelData, &ptVideoMem->tPixelDatas);
                }
...
static int GenerateDirAndFileIcons(PT_PageLayout ptPageLayout)
{
    ...
    for(i=0;i<ARRAYSIZE(g_TfileIconDesc);i++) {
        g_TfileIconDesc[i].tPixeldatas.iBpp          = iBpp;
        g_TfileIconDesc[i].tPixeldatas.aucPixelDatas = malloc(ptPageLayout->iMaxTotalBytes);
        if (g_TfileIconDesc[i].tPixeldatas.aucPixelDatas == NULL)
        {
            return -1;
        }
    }
    ...
    // xhr unknowfile
    iError = GetPixelDatasForIcon(UNKNOW_FILETYPE_ICON, &tOriginIconPixelDatas);
    if (iError)
    {
        DBG_PRINTF("GetPixelDatasForIcon unknowfile %s error!\n", UNKNOW_FILETYPE_ICON);
        return -1;
    }
    ...
    /* 3. 提取"file图标" */
    for(i=0;i<ARRAYSIZE(g_TfileIconDesc);i++) {
        iError = GetPixelDatasForIcon(g_TfileIconDesc[i].file, &tOriginIconPixelDatas);
        if (iError)
        {
            DBG_PRINTF("GetPixelDatasForIcon %d %s error!\n",i, g_TfileIconDesc[i].file);
            return -1;
        }

        g_TfileIconDesc[i].tPixeldatas.iHeight = atLayout[0].iBotRightY - atLayout[0].iTopLeftY + 1;
        g_TfileIconDesc[i].tPixeldatas.iWidth  = atLayout[0].iBotRightX - atLayout[0].iTopLeftX+ 1;
        g_TfileIconDesc[i].tPixeldatas.iLineBytes  = g_tDirClosedIconPixelDatas.iWidth * g_tDirClosedIconPixelDatas.iBpp / 8;
        g_TfileIconDesc[i].tPixeldatas.iTotalBytes = g_TfileIconDesc[i].tPixeldatas.iLineBytes * g_TfileIconDesc[i].tPixeldatas.iHeight;
        PicZoom(&tOriginIconPixelDatas, &g_TfileIconDesc[i].tPixeldatas);
        FreePixelDatasForIcon(&tOriginIconPixelDatas);
    }
    return 0;
}
...
static void ShowBrowsePage(PT_PageLayout ptPageLayout)
{
    ...
    {
        if(GenerateDirAndFileIcons(&g_tBrowsePageDirAndFileLayout)) {
            DBG_PRINTF("%s %s GenerateDirAndFileIcons error!\n",__FILE__,__FUNCTION__);
            return;
        }
    }
